...
int(timer_test_read_config)(uint8_t timer, enum timer_status_field field) {
  //ler as configurações do timer e dar print
  uint8_t st;
...
int(timer_test_int)(uint8_t time) {
  //não é preciso saber o hook_id que atribuíste, apenas perguntas ao dispositivo que mascara é que ele vai usar (0 para o timer, 1 para o keyboard)
  int ipc_status, r;
  uint8_t irq_set;
  message msg;
  if (timer_subscribe_int(&irq_set)) return 1;
  //copiar código so Souto
